#!/bin/bash
#设定环境变量，因为不能保障运行的时候的shell脚本的环境变量都是ok的
#环境变量有俩种一个是java；；一个是hadoop
#set java env
...
log_src_dir=/usr/local/hadoop/logs/hdfslog/log/

#待上传文件存放的目录
#log_toupload_dir=/home/hadoop/logs/toupload/
log_toupload_dir=/usr/local/hadoop/logs/hdfslog/toupload/

#日志文件上传到hdfs的根路径
hdfs_root_dir=/data/clickLog/20180518/
